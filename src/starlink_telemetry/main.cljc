(ns starlink_telemetry.main
  "Kotodama WASM entrypoint for the Starlink_telemetry clean-room actor (L4) — Clojure port.
  ...
  facts (`starlink_telemetry.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "starlink_telemetry")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"starlink_telemetry.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "starlink_telemetry-compat" :tier tier :entities entities} 200])
...
(defn start! [] :starlink_telemetry-compat/ready)
