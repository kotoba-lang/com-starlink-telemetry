(ns starlink_telemetry.main-test
  "Contract + behavioral test for the starlink_telemetry-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "starlink_telemetry-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'starlink_telemetry.main-test)]
